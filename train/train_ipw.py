...
def main():
    ...
    # Model names
    best_propensity_model_name = f'unetlstm_propensity_dim_horizon_{dim_horizon}_beta_1_{beta_1:.1f}.pth'
    best_model_name = f'unetlstm_ipw_dim_horizon_{dim_horizon}_beta_1_{beta_1:.1f}.pth'
    ...
    # Data loader
    train_loader_raw = torch.utils.data.DataLoader(dataset=train_dataset,
                                                   batch_size=batch_size,
                                                   shuffle=False)
    test_loader_raw = torch.utils.data.DataLoader(dataset=test_dataset,
                                                  batch_size=batch_size,
                                                  shuffle=False)
    
    # Data normalization
    normalizer = data_utils.DataNormalizer(train_loader_raw)
    train_loader_normalized, test_loader_normalized = normalizer.normalize(train_loader_raw, test_loader_raw)
    ...
    for epoch in range(num_epochs):
        ...
        for i, ((x_norm, A_norm, Y_norm), (x_raw, A_raw, Y_raw)) in enumerate(zip(train_loader_normalized, train_loader_raw)):
            b = x_norm.size(0)
            ...
            A_past_norm = A_norm[:, :tlen].reshape(b, tlen, 1, height, width)
            Y_past_norm = Y_norm[:, :tlen].reshape(b, tlen, 1, height, width)
            ...
            logits = propensity_model(inputs).reshape(b, height, width)
            loss = criterion_prop(logits, A_curr_unnorm)
        ...
        with torch.no_grad():
            test_loss = 0
            num_test = 0
            for i, ((x_norm, A_norm, Y_norm), (x_raw, A_raw, Y_raw)) in enumerate(zip(test_loader_normalized, test_loader_raw)):
                b = x_norm.size(0)
                ...
                logits = propensity_model(inputs).reshape(b, height, width)
                batch_loss = criterion_prop(logits, A_curr_unnorm)
                test_loss += batch_loss.item() * b
                num_test += b
    ...
    for epoch in range(num_epochs):
        ...
        for i, ((x_norm, A_norm, Y_norm), (x_raw, A_raw, Y_raw)) in enumerate(zip(train_loader_normalized, train_loader_raw)):
            ...
            loss = criterion(outputs, Y_out_ipw)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for i, ((x_norm, A_norm, Y_norm), (x_raw, A_raw, Y_raw)) in enumerate(zip(test_loader_normalized, test_loader_raw)):
                ...
                # IPW model predicts the final outcome
                outputs = ipw_model(inputs_ipw).reshape(b, height, width)
                batch_loss = criterion(outputs, Y_out_ipw)
    ...
    if use_best_model:
        ...
        ipw_model.eval()
        outputs = ipw_model(inputs).squeeze(1)
    ...
    save_params(params_dict, os.path.join(models_dir, f"ipwunet_best_loss_dim_horizon_{dim_horizon}.txt"))
